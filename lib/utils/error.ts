...
export function handleFirebaseError(err: unknown): AppError {
  console.error('[Firebase Diagnostic Error Trace]:', err);
  logger.error('AUTH', 'Captured Firebase Auth Exception:', err);

  if (isFirebaseError(err)) {
    const rawMsg = err.message || '';
    const code = err.code || (rawMsg.includes('CONFIGURATION_NOT_FOUND') ? 'auth/configuration-not-found' : 'auth/unknown');
    let explanation = '';

    if (rawMsg.includes('CONFIGURATION_NOT_FOUND') || code === 'auth/configuration-not-found') {
      explanation = 'Firebase Authentication is not activated in project analyticsrise-56655. Enable Email/Password provider in Firebase Console (Authentication -> Get Started -> Email/Password).';
    } else {
      switch (code) {
        ...
        case 'permission-denied':
          ...
          break;
        default:
          explanation = rawMsg || 'Firebase operation failed.';
          break;
      }
    }
